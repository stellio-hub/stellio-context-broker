...
object QueryUtils {
    ...
    private fun buildInnerQuery(rawQuery: String, jsonLdEntity: JsonLdEntity, contexts: List<String>): String {
        ...
        return rawQueryWithPatternEscaped.replace(qPattern.toRegex()) { matchResult ->
            ...
            val targetValue = query.third.quoteIfNeeded().replaceSimpleQuote()
    ...
    private fun getAttributeType(expandedAttribute: ExpandedTerm, jsonLdEntity: JsonLdEntity): AttributeType {
        val jsonLdAttribute = jsonLdEntity.properties[expandedAttribute]
        return if (jsonLdAttribute != null) {
            AttributeType(
                (
                    (
                        (
            ...
            throw BadRequestDataException("Unmatched query since it contains an unknown attribute $expandedAttribute")
    }
