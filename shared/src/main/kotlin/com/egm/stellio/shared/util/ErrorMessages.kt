 ...
 * docs/contributing/error-messages-guideline.md. All messages use:
...
object DataRepresentationErrorMessages {
...
object EntityErrorMessages {
    ...
    const val ENTITY_ID_MISMATCH_MESSAGE = "The id contained in the body is not the same as the one provided in the URL"
...
object EntityTypeInfoErrorMessages {
    fun typeNotFoundMessage(type: String) = "Type $type does not exist"
}

object GenericValidationErrorMessages {
    ...
    fun cannotSpecifyWhenPresent(field: String, vararg fields: String) =
        "Cannot specify '$field' when ${fields.joinToString(" or ") { "'$it'" }} are present"

    fun invalidGeometryDefinitionMessage(geoJsonPayload: String, error: String) =
        "Invalid geometry definition: $geoJsonPayload ($error)"
}

object HttpRequestErrorMessages {
...
object JsonLdContextServerErrorMessages {
...
object QueryParameterErrorMessages {
    ...
    fun invalidIdPatternMessage(idPattern: String?) = "Invalid pattern in field 'idPattern': $idPattern"
    const val INVALID_ID_PATTERN_MESSAGE = "Invalid pattern in field 'idPattern'"
    ...
    fun invalidOptionsValueMessage(option: String) = "'$option' is not a valid value for the options query parameter"
    fun invalidOrderingDirectionMessage(direction: String) = "'$direction' is not a valid ordering direction parameter"
    ...
    const val PROJECTION_SEPARATOR_AFTER_BRACE_MESSAGE = "Expression cannot contain a separator after an opening brace"
...
object SubscriptionErrorMessages {
    ...
    const val SHOW_CHANGES_WITH_SIMPLIFIED_MESSAGE = "Cannot use 'showChanges' with 'simplified' or 'keyValues' format"
}
...
object TemporalErrorMessages {
    ...
    const val AGGR_METHODS_MANDATORY_MESSAGE = "'aggrMethods' is mandatory if 'aggregatedValues' option is specified"
