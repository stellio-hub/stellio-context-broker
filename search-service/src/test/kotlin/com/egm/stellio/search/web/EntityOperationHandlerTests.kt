...
class EntityOperationHandlerTests {
    ...
    fun `create batch entity should return a 201 if JSON-LD payload is correct`() = runTest {
        ...
        coEvery { authorizationService.createCreatorRights(any(), eq(sub)) } returns Unit.right()
        coEvery {
        ...
        coVerify { authorizationService.createCreatorRights(allEntitiesUris, sub) }
    ...
    fun `create batch entity should return a 207 when some entities already exist`() = runTest {
        ...
        coVerify { authorizationService.createCreatorRights(createdEntitiesIds, sub) }
    ...
    fun `upsert batch entity should return a 207 if JSON-LD payload contains update errors`() = runTest {
        ...
        coVerify { authorizationService.createCreatorRights(listOf(deviceUri), sub) }
