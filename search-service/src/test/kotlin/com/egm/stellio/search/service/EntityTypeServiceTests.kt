...
class EntityTypeServiceTests : WithTimescaleContainer, WithKafkaContainer {
    ...
    fun `it should error when type doesn't exist`() = runTest {
        val entityTypeInfo = entityTypeService.getEntityTypeInfoByType("test", listOf(APIC_COMPOUND_CONTEXT))

        entityTypeInfo.shouldFail {
            assertEquals(ResourceNotFoundException(typeNotFoundMessage("test")), it)
        }
    }
