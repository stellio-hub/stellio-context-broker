...
class EntityHandlerTests {
    ...
    fun `should return a 201 if JSON-LD payload is correct`() {
        ...
        every { neo4jRepository.createOrUpdateEntity(any(), any()) } returns "urn:ngsi-ld:BeeHive:TESTC"
    ...
    fun `should return a 409 if the entity is already existing`() {
        val jsonLdFile = ClassPathResource("/ngsild/beehive.json")
        every { neo4jRepository.createOrUpdateEntity(any(), any()) } throws AlreadyExistingEntityException("already existing entity urn:ngsi-ld:BeeHive:TESTC")
