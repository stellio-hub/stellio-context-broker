...
object ConciseRepresentationUtils {
    ...
    fun normalizeAttributeFragment(value: Any): Map<String, Any> =
        when (value) {
            is String, is Number, is Boolean ->
                mapOf(NGSILD_TYPE_TERM to NGSILD_PROPERTY_TERM, NGSILD_VALUE_TERM to value)
            is Map<*, *> -> {
                ...
                when {
                    ...
                    // treated as a normalized representation.
                    // Only said in 4.5.2.3 but apply for all kinds of attributes.
                    NGSILD_ATTRIBUTE_TYPES.contains(attributeType) ->
                        compactedAttributeInstance
                    isGeoJsonGeometry(compactedAttributeInstance) ->
                        mapOf(
                            NGSILD_TYPE_TERM to NGSILD_GEOPROPERTY_TERM,
                            NGSILD_VALUE_TERM to compactedAttributeInstance
                        )
                    ...
                    compactedAttributeInstance.containsKey(NGSILD_LANGUAGEMAP_TERM) ->
                        normalizeSubAttrs(
                            compactedAttributeInstance.plus(NGSILD_TYPE_TERM to NGSILD_LANGUAGEPROPERTY_TERM)
                        )
