...
The EGM image gives you access to the [subject endpoints](../user/authentication_and_authorization.md#subjects-endpoints)
 and allows the [`on-owner-delete-cascade-entities` property](../user/authentication_and_authorization.md#delete-entities-owned-by-stellio-user-if-said-user-is-deleted).
...
- `STELLIO_AUTHENTICATION_CLAIMS_PATHS`: `my.claim.path,my.other.claim.path` 
  - A comma-separated list of paths, the path should correspond to a claim in the JWT generated by your OIDC provider.
  - When [evaluating permissions](../user/authentication_and_authorization.md/#manage-access-rights),
    the specified claims will be used to see if the user gets access to the resource.
