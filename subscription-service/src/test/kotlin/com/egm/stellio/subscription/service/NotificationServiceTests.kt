...
class NotificationServiceTests {
    ...
    fun `it should notify the subscriber and update the subscription`() = runTest {
        ...
        coEvery { subscriptionService.getMatchingSubscriptions(any(), any()) } returns listOf(subscription)
        coEvery { subscriptionService.isMatchingTypesQuery(any(), any()) } returns true.right()
        ...
        coVerify { subscriptionService.isMatchingTypesQuery(subscription.entities.map { it.type }, any()) }
    ...
    fun `it should notify the two subscribers`() = runTest {
        ...
        coEvery {
            subscriptionService.getMatchingSubscriptions(any(), any())
        ...
        coVerify { subscriptionService.isMatchingTypesQuery(subscription1.entities.map { it.type }, any()) }
        coVerify { subscriptionService.isMatchingTypesQuery(subscription2.entities.map { it.type }, any()) }
