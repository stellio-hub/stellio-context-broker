...
class ServiceExecutionHandlerTests {
    ...
    @Test
    fun `create should return 201 with the execution result`() = runTest {
        coEvery { serviceExecutionLauncher.invokeService(any()) } returns execution.right()
    ...
    @Test
    fun `create should propagate launcher rejection of result members`() = runTest {
        coEvery {
            serviceExecutionLauncher.invokeService(any())
        } returns BadRequestDataException("invalid result member").left()
        ...
        coVerify(exactly = 4) { serviceExecutionLauncher.invokeService(any()) }
    }

    @Test
    fun `create should propagate an input validation failure`() = runTest {
        coEvery {
            serviceExecutionLauncher.invokeService(any())
        } returns BadRequestDataException("invalid input").left()
