...
class Neo4jRepository(
    ...
    fun getEntityTypeAttributesInformation(expandedType: String): Map<String, Any> {
        ...
        return mapOf(
            ...
            "geoProperties" to
                (if (entityWithLocationCount > 0) setOf(NGSILD_LOCATION_PROPERTY) else emptySet()).plus(
                    if (entityWithOperationSpaceCount > 0) setOf(NGSILD_OPERATION_SPACE_PROPERTY)
                    else emptySet()
                ).plus(
                    if (entityWithObservationSpaceCount > 0) setOf(NGSILD_OBSERVATION_SPACE_PROPERTY)
                    else emptySet()
                ),
